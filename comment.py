...
class sina_comments(object):
    def __init__(self):
        ...
        self.since_id_list = []                     #since_id列表，每一次获取的since_id
        self.replyList = []                         #回复列表，以weiboID_commentID列表格式保存
        
        
    #1、申请获取Oauth2授权方式的code
    def Get_Oauth2Code(self):
        client = APIClient(app_key=self.APP_KEY, app_secret=self.APP_SECRET, redirect_uri=self.CALLBACK_URL)
        ...
        for weiboID, commentID_list in IDdict.items():
            for commentID in commentID_list:
                ...
                try:
                    ...
                    print("*【回复id】\t%d，【回复内容】\t%s"%(commentID,reply_mes_random))
                    time.sleep(5)
                except:
                    print("*【回复id】\t%d失败"%commentID)
        print("完成本次自动回复工作！",time.ctime())
        return
    
    #7、获取最新的未回复的since_id，以最晚发出的评论id为since_id
    def Get_CommentsByMe(self):
        comment_send = self.client.comments__by_me().comments
        return comment_send
    
    #8、获取并保存since_id
    def Get_sinceID(self, comments):
        since_id =  comments[0].id
        self.since_id_list.append(since_id)
        return since_id
    ...
    def comments_train(self):
        ...
        try:
            while flag:
                if self.since_id == 0:
                    comments_send = self.Get_CommentsByMe()
                    self.since_id = self.Get_sinceID(comments_send)
                    print("【since_id】since_id为0，使用最晚发出的评论id：\n",comment_send[0].id)
                comments = self.Get_CommentsToMe(self.since_id)
                if len(comments) == 0:
                    print("^^^^^^^^^^^^\n暂无更新评论！")
                    print("【since_id】最新的since_id是：",self.since_id)
                else:
                    print("^^^^^^^^^^^^\n有%d条更新评论！"%len(comments))
                    print("【since_id】原来的since_id是：",self.since_id)
                    self.since_id = self.Get_sinceID(comments)
                    print("【since_id】最新的since_id是：",self.since_id)
                    weibo_comments = self.Show_CommentsToMe(comments)
                    self.AutoReply(weibo_comments)
                flag = int(input("\n是否刷新评论并自动回复？\"是\"输入1，\"否\"输入0"))
        except:
            print("授权过期，重新申请授权！")
            self.Get_Oauth2Code()
            self.Get_token()
            ...
            while flag:
                if self.since_id == 0:
                    comments_send = self.Get_CommentsByMe()
                    self.since_id = self.Get_sinceID(comments_send)
                    print("【since_id】since_id为0，使用最晚发出的评论id：",comment_send[0].id)
                comments = self.Get_CommentsToMe(self.since_id)
                if len(comments) == 0:
                    print("^^^^^^^^^^^^\n暂无更新评论！")
                    print("【since_id】最新的since_id是：",self.since_id)
